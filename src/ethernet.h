...
struct Generic_Link_layer : public Link_layer, Source_address {
        ...
        Generic_Link_layer(size_t const header_length, ether_addr const source, uint16_t const payload_protocol, std::string const info);
...
struct Linux_cooked_capture : public Link_layer, Source_address {
        ...
        std::array<uint8_t, 8> source_address{{0}};
        uint16_t payload_type;

        public:
        template<typename iterator>
        Linux_cooked_capture(iterator data, iterator end) {
                check_type_and_range(data, end, header_length());
                std::advance(data, 2);
...
struct VLAN_Header : public Link_layer {
        ...
        public:
        template<typename iterator>
        VLAN_Header(iterator data, iterator end) {
                check_type_and_range(data, end, header_length());
                std::advance(data, 2);
                payload_type = ntohs(*reinterpret_cast<uint16_t const *>(&(*data)));
        }
...
struct sniff_ethernet : public Link_layer, Source_address {
         ...
         */
        template<typename iterator>
        sniff_ethernet(iterator data, iterator end) {
                check_type_and_range(data, end, header_length());
...
std::unique_ptr<Link_layer> parse_linux_cooked_capture(iterator data, iterator end) {
        ...
        return std::unique_ptr<Link_layer>(new Generic_Link_layer(header_size, ether_shost, payload_type, info));
}
...
std::unique_ptr<Link_layer> parse_ethernet(iterator data, iterator end) {
        ...
        return std::unique_ptr<Link_layer>(new Generic_Link_layer(header_size, ether_shost, ether_type, info));
}
...
std::unique_ptr<Link_layer> parse_VLAN_Header(iterator data, iterator end) {
        ...
        return std::unique_ptr<Link_layer>(new Generic_Link_layer(header_size, ether_shost, payload_type, "VLAN Header"));
}

template<typename iterator>
std::unique_ptr<Link_layer> parse_link_layer_new(const int type, iterator data, iterator end) {
        switch (type) {
