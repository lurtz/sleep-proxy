...
struct Socket_listen : public Socket {
        Socket_listen(int domain, int type, int protocol = 0) : Socket(domain, type, protocol) {}

        template<typename Sockaddr>
        void bind(Sockaddr&& sockaddr) {
                const int ret_val = ::bind(sock, reinterpret_cast<const struct sockaddr *>(&sockaddr), sizeof(Sockaddr));
                if (ret_val != 0) {
        ...
        Optval get_sock_opt(int level, int optname) const {
                Optval optval;
                socklen_t optlen = sizeof(Optval);
                const int ret_val = getsockopt(sock, level, optname, &optval, &optlen);
                if (ret_val) {
                        throw std::runtime_error(std::string("getsockopt() failed: ") + strerror(errno));
                }
                return optval;
        }
